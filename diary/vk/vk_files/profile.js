var Profile = {
  ...
  frDropdownPreload: function(sh) {
    ...
    ajax.post('al_friends.php', {act: 'friend_dd', mid: cur.oid}, {onDone: function(html, js) {
      ...
      var el = ge('profile_frdd_wrap');
      domPN(el).insertBefore(domFC(ce('div', {innerHTML: html})), el);
      eval(js);
    }, cache: 1});
  },
  frDropdown: function() {
    Profile.frDropdownPreload(1);
  },
  ...
  showGiftBox: function(mid, ev) {
    ...
    return !showBox('al_gifts.php', {act: 'get_gift_box', mid: mid, fr: (mid == vk.id ? 1 : 0)}, {stat: ['gifts.css', 'wide_dd.js', 'wide_dd.css'], cache: 1}, ev);
  },
