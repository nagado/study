...
AdsLight.showNewBlock = function(containerElem, adsHtml, isContainerVisible) {
  ...
  var speed            = ((isNewBlockEmpty || containerElemY + 50 > scrollY + lastWindowHeight) ? 0 : 200);
...
AdsLight.addAdsToApply = function(adsIdsApply) {
  for (var elemId in adsIdsApply) {
    vk__adsLight.adsIdsApplyNeeded[elemId] = adsIdsApply[elemId];
  }
  AdsLight.applyAds();
}

AdsLight.applyAds = function(delayed) {
  if (isEmpty(vk__adsLight.adsIdsApplyNeeded)) {
    return;
  }
  if (!delayed) {
  ...
  function check() {
    for (var elemId in vk__adsLight.adsIdsApplyNeeded) {
      var elem = ge(elemId);
      if (!elem) {
        delete vk__adsLight.adsIdsApplyNeeded[elemId];
        continue;
      }
      elemRect = elem.getBoundingClientRect();
      if (elemRect.bottom > 0 && elemRect.top < lastWindowHeight) {
        vk__adsLight.adsIdsApplyProcess.push(vk__adsLight.adsIdsApplyNeeded[elemId][0]);
        delete vk__adsLight.adsIdsApplyNeeded[elemId];
      } else if (vk__adsLight.adsIdsApplyNeeded[elemId][1] && (vkNow() - vk.started) / 1000 > vk__adsLight.adsIdsApplyNeeded[elemId][1]) {
        re(elem);
        delete vk__adsLight.adsIdsApplyNeeded[elemId];
      }
    }
...
AdsLight.handleAllAds = function(box, adsIdsMore, adsIdsApply, adsHeightMore) {
  ...
  function checkDeinit() {
    if (!adsIdsMore && isEmpty(vk__adsLight.adsIdsApplyNeeded)) {
      deinit();
    }
  }
